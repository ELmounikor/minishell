...
t_list	*check(t_list *previous, t_list *current, t_list *head, t_env *env)
{
	...
	!((current->content && current->content[0]) || (current->next && \
	(current->next->id != 44 && current->next->id != 4 && current->next->id \
	!= 440 && current->next->id != 40 && current->next->id != 770 && \
	current->next->id != 70 && current->next->id != 77 && current->next->id \
	!= 7 && current->next->id != 1))))
